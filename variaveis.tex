...
\author{JOÃO VICTOR DA CRUZ SILVESTRE}  % Seu nome
\authorbib{Silvestre, João Victor} % Seu nome para referência bibliográfica (Sobrenome, Nome)
\title{INVESTIGAÇÃO DA VIABILIDADE DE DISTRIBUIÇÃO DE UMA APLICAÇÃO PROVA DE CONCEITO BASEADA NO PARADIGMA ORIENTADO A NOTIFICAÇÕES EM UM CLUSTER KUBERNETES.} % Título do trabalho
...
\titleen{Investigation of the feasibility of distributing a proof-of-concept application based on the notification-driven paradigm in a Kubernetes cluster.} % Título traduzido para inglês
\advisor{Prof. Dr. Clayton Kossoski} % Nome do orientador. Lembre-se de prefixar com "Prof. Dr.", "Profª. Drª.", "Prof. Me." ou "Profª. Me."}
...
\PalavraChaveA{paradigma orientado a notifica\c{c}\~oes}%% Palavra-chave A
\PalavraChaveB{Kubernetes}%% Palavra-chave B
\PalavraChaveC{sistemas distribu\´idos}%% Palavra-chave C
\PalavraChaveD{conteineriza\c{c}\~ao}%% Palavra-chave D
\PalavraChaveE{viabilidade t\'ecnica}%% Palavra-chave E
...
\KeywordB{Kubernetes}%% Keyword B
\KeywordC{distributed systems}%% Keyword C
\KeywordD{containerization}%% Keyword D
\KeywordE{technical feasibility}%% Keyword E
